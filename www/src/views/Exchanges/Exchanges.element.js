var WtcExchanges = Ractive.extend({
    oninit: function () {
        console.log('Exchanges.oninit !');
        ractiveComponent['wtc-ExchangesApp'].set('MyProfile', MyProfile);
        Messenger.auth_action.call_wait_auth(function () {
            Messenger.threads.callback_save('ProfileApp', function (threads) {
                ractiveComponent['wtc-ExchangesApp'].set('threads', threads);
                ractiveComponent['wtc-ExchangesApp'].set('threads_finish', Messenger.threads.finish);
                $(".__nano").nanoScroller();
            });
        });
        $(".__nano").nanoScroller();
